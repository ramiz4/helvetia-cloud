...
export function UsageMetrics({ usage }: UsageMetricsProps) {
  const getIcon = (metric: string) => {
    switch (metric) {
      case 'COMPUTE_HOURS':
        return <Activity size={24} className="text-indigo-400" />;
      case 'MEMORY_GB_HOURS':
        return <Database size={24} className="text-purple-400" />;
      case 'BANDWIDTH_GB':
        return <Network size={24} className="text-cyan-400" />;
      case 'STORAGE_GB':
        return <HardDrive size={24} className="text-emerald-400" />;
      default:
  ...
  return (
    <div className="p-8 rounded-[32px] bg-slate-900/40 backdrop-blur-xl border border-white/10 shadow-2xl">
      <div className="mb-6">
        <h3 className="text-2xl font-bold text-white mb-2">Usage Metrics</h3>
      ...
      {usage.usage.length === 0 && (
        <div className="text-center py-12">
          <p className="text-slate-400 font-medium">No usage data available for this period</p>
        </div>
      )}
    </div>
  );
